...
RUN git clone https://github.com/barabo/advanced-shell-history.git  \
  && cd advanced-shell-history \
  && make build \
  && make install \
  && echo "source /usr/local/etc/advanced-shell-history/config" >> ~/.profile \
  && echo "export ASH_CFG_SYSTEM_QUERY_FILE" >> ~/.profile \
  && echo "source /usr/local/lib/advanced_shell_history/sh/zsh" >> ~/.profile
